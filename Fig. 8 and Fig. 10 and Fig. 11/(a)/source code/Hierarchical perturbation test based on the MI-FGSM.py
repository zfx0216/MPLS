...
def update_gradient_momentum(grad, prev_grad):
    grad = grad / (np.mean(np.abs(grad)) + 1e-12)
...
model = alexnet(num_classes=50).to(device)
...
def run_one_config(cfg, image_files):
    ...
    for image_file in image_files:
        ...
        stop_flag = False
        valid_images = []
        prev_grad = np.zeros((3, 224, 224), dtype=np.float32)
        ...
        while not stop_flag and iter_count <= max_num_iterative:
            ...
            grad = update_gradient_momentum(grad, prev_grad)
            prev_grad = grad.copy()
