...
tampering_quantity = int(0.2 * 3 * 224 * 224)
...
for image_file in image_files:
    ...
    while flag == 0 and num_iterative <= max_num_iterative:
        ...
        for i, j in zip(rows_r, cols_r):
            g = iterative_image_pixel_weight_matrix_in_attack_label_R[i][j]
            if g == 0:
                continue
        ...
        for i, j in zip(rows_g, cols_g):
            g = iterative_image_pixel_weight_matrix_in_attack_label_G[i][j]
            if g == 0:
                continue
            total_marked_pixels += 1
        ...
        for i, j in zip(rows_b, cols_b):
            g = iterative_image_pixel_weight_matrix_in_attack_label_B[i][j]
            if g == 0:
                continue
            total_marked_pixels += 1
        ...
        if solver.check() == z3.sat:
            ...
            for i, j in zip(rows_r, cols_r):
                ...
                if var is not None:
                    val_fp = z3_model[var]
                    val_real = z3.fpToReal(val_fp)
                    val_num = z3_model.eval(val_real)

                    res_fraction = val_num.as_fraction()
                    delta_float = float(res_fraction.numerator) / float(res_fraction.denominator)
