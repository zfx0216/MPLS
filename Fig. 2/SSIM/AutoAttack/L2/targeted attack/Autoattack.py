...
class APGDAttack():
    ...
    def attack_single_run(self, x, y, x_init=None):
        ...
        x_adv[0][0] = torch.clamp(x_adv[0][0], min=-2.1179, max=2.2489).detach()
        x_adv[0][1] = torch.clamp(x_adv[0][1], min=-2.0357, max=2.4285).detach()
        x_adv[0][2] = torch.clamp(x_adv[0][2], min=-1.8044, max=2.64).detach()
        ...
        for i in range(self.n_iter):
            ...
            with torch.no_grad():
                ...
                if self.norm == 'Linf':
                    ...
                    # x_adv_1 = torch.clamp(x + self.normalize(x_adv_1 - x) * torch.min(self.eps * torch.ones_like(x).detach(), L2_norm(x_adv_1 - x, keepdim=True)), -2.117, 2.24890)
...
class APGDAttack_targeted(APGDAttack):
    def __init__(
        ...
        self.y_target = None   # 保存目标类别（你想诱导模型预测成的类别）
        self.n_target_classes = n_target_classes

    def dlr_loss_targeted(self, x, y):             # 这是 DLR 有目标损失函数，它的目标是：让目标类别得分更高，原始类别得分更低。
        x_sorted, ind_sorted = x.sort(dim=1)
        u = torch.arange(x.shape[0])
    ...
    def perturb(self, x, target_label):
        """
        对输入图像 x 进行有目标攻击，目标标签为 target_label。

        :param x: 原始图像 (N, C, H, W)
        :param target_label: 目标标签 (N,)
        :return: 对抗样本 (N, C, H, W)
        """
        assert self.loss in ['ce-targeted'], "当前仅支持 'dlr-targeted' 损失" 'ce-targeted'

        # 初始化
        self.init_hyperparam(x)
        x = x.detach().clone().float().to(self.device)
        target_label = target_label.detach().clone().long().to(self.device)
        ...
        # 攻击样本初始化
        adv = x.clone()
        acc = y_pred == self.y_target  # 目标攻击成功的样本

        if self.verbose:
            print(f"正在进行 targeted 攻击（目标类别: {target_label.tolist()}），ε = {self.eps}")
            print('初始预测类别:', y_pred.tolist())
            print('目标类别:', target_label.tolist())
        ...
        for counter in range(self.n_restarts):
            x_to_attack = x.clone()
            y_to_attack = y_pred.clone()  # 实际标签仅用于内部记录

            if not self.use_largereps:
                _, _, _, adv_curr = self.attack_single_run(x_to_attack, y_to_attack)
            else:
                epss = [3. * self.eps_orig, 2. * self.eps_orig, 1. * self.eps_orig]
                iters = [.3 * self.n_iter_orig, .3 * self.n_iter_orig, .4 * self.n_iter_orig]
                iters = [math.ceil(c) for c in iters]
                iters[-1] = self.n_iter_orig - sum(iters[:-1])
                _, _, _, adv_curr = self.decr_eps_pgd(x_to_attack, y_to_attack, epss, iters)

            adv = adv_curr.detach()

            adv[0][0] = torch.clamp(adv[0][0], min=-2.1179, max=2.2489).detach()
            adv[0][1] = torch.clamp(adv[0][1], min=-2.0357, max=2.4285).detach()
            adv[0][2] = torch.clamp(adv[0][2], min=-1.8044, max=2.64).detach()
